...
from controls.estimation.kalman import KalmanFilter, KalmanObservation
...
def test_assertions():
    ...
    process_noise = np.array([[0, 0, 0], [0, 0.1, 0], [0, 0.8, 0]])
...
def test_noisey_1d():
    ...
    for i in range(1, 100):
        obs = F @ obs
        kalman_filter.observe(
            KalmanObservation(Y=obs[0] + np.random.normal(0, 1e-8), U=np.array([[0]]))
        )
...
def test_wrong_noisey_1d():
    ...
    for i in range(1, 200):
        obs = F @ obs
        kalman_filter.observe(
            KalmanObservation(Y=obs[0] + np.random.normal(0, 1e-5), U=np.array([[0]]))
        )
